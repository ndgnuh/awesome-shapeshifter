...
local hasthisdir, thisdir = pcall(require, "thisdir")
local hasfennel, fennel = pcall(require, "fennel.fennel")
--}}}

-- directories {{{
local wmdir = gears.filesystem.get_configuration_dir()
local ricedir = hasthisdir and thisdir() or (wmdir .. "/Rice/")
...
module.load_rice = function(self, ricename)
	...
	local addtopath = {
		...
		ricedir .. "/?/init.",
		ricedir .. "/" .. ricename .. "/?.",
		ricedir .. "/" .. ricename .. "/?/init.",
	}
	for _, dir in ipairs(addtopath) do
		package.path = wmdir .. dir .. "lua" .. ";" .. package.path
		if hasfennel then
			fennel.path = wmdir .. dir .. "fnl" .. ";" .. fennel.path
